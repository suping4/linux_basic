...
void saveImage(unsigned char *inimg) {
    ...
    fwrite((void*)&bmpFileHeader, sizeof(BITMAPFILEHEADER), 1, fp);
    fwrite((void*)&bmpInfoHeader, sizeof(BITMAPINFOHEADER), 1, fp);
    
    fwrite((void*)palrgb, sizeof(RGBQUAD), 256, fp);
...
static void process_image(const void *p)
{
    ...
    unsigned char inimg[WIDTH*HEIGHT*NUMCOLOR];
    ...
    for(y=0; y<height; y++, in+=istride, count=0) {
        for(j=0; j<vinfo.xres * 2; j+=colors) {
            ...
            inimg[(height-1-y)*width*NUMCOLOR + count++] = b;
            inimg[(height-1-y)*width*NUMCOLOR + count++] = g;
            inimg[(height-1-y)*width*NUMCOLOR + count++] = r;
...
void saveImage(unsigned char *inimg) {
    ...
    fwrite(inimg, WIDTH*HEIGHT*NUMCOLOR, 1, fp);
    fclose(fp);
}
